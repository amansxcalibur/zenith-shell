...
def i3_border_setter(hex_color: str = None, reload: bool = False):
    if hex_color is None:
        raw_color = get_css_variable(f"{ROOT_DIR}/styles/colors.css", "--primary")
...
def generate_i3_keybinds(bindings: list[KeyBinding]) -> str:
    lines = [
        ...
        "# To change these bindings, you can Edit I3_KEYBINDINGS in",
...
def get_i3_config_path() -> Path:
    try:
        # Use your existing get_i3_connection to get the singleton
...
def i3_keybinds_setter(reload: bool = False):
...
def generate_i3_gaps_and_corners():
    lines = [
        "# CAUTION: This file has been auto-generated by Zenith Shell.",
        "# Any manual changes WILL BE OVERWRITTEN on the next startup."
        "# To change these bindings, you can edit the config from the "
        f"# Settings or in {ROOT_DIR}/config/config.json",
        "",
    ]
...
def i3_borders_and_gaps_setter(reload: bool = False):
    ...
    try:
        ensure_i3_config_includes_glob_dir(main_config_path, CONFIG_GLOB_PATH)
        config = generate_i3_gaps_and_corners()
