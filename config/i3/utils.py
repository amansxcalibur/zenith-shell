...
def generate_i3_keybinds(bindings: list[KeyBinding]) -> str:
    lines = [
        ...
        "# To change these bindings, you can:",
        f"# 1. Edit I3_KEYBINDINGS in {ROOT_DIR}/config/bindings.py",
        "# 2. Define the same key in another .conf file loaded AFTER this one",
        "#    to override the Zenith default.",
        "",
    ]
...
def get_i3_config_path():
    try:
        ...
        return Path(version.loaded_config_file_name)
    except Exception as e:
        logger.error("Could not determine i3 config path via IPC: {}", e)
