...
class PlayerService(Service):
    ...
    def __init__(self, player: Playerctl.Player, **kwargs):
        ...
        if player.props.player_name in config.ALLOWED_PLAYERS:
            ...
            logger.info(f"Connected {len(self._signal_ids)} signals for {player.props.player_name}")
    ...
    def on_metadata(self, player, metadata):
        ...
        if "xesam:artist" in keys and "xesam:title" in keys:
            logger.info(
                f"{metadata['xesam:artist'][0]} - {metadata['xesam:title']}"
            )
    ...
    def cleanup(self):
        ...
        if self._is_cleaning_up:
            logger.warning(f"Cleanup already in progress for {self._player.props.player_name}")
            return
